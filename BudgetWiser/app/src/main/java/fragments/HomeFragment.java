...
public class HomeFragment extends Fragment {
    ...
    private ArrayList<PieEntry> getDataValue() {
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            cursor.moveToNext();
            //Todo-- check, if "category" is same from previous dataValue, SUM to that dataValue, else continue as usual to new dataValue;
            dataValue.add(new PieEntry(cursor.getFloat(0), cursor.getString(1)));
        }
